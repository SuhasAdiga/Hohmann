...
class Planet():
    ...
    def calculateInitialVelocity(self, delta):
        ...
        return (velocity * np.cos(self.eccentricAnomaly) - self.position[0]/self.distanceToSun()**3 * delta/2,
                velocity * np.sin(self.eccentricAnomaly) - self.position[1]/self.distanceToSun()**3 * delta/2)
    ...
    def updateVelocity(self, delta):
        ...
        return tuple(i - (delta * ri) / self.distanceToSun()**3 for i, ri in zip(self.velocity, self.position))
...
class SolarSystemSimulation():
    ...
    def __init__(self):
        ...
        self.delta = gui.stepsize.get() * 2*np.pi / 365.256   # integration timestep in Earth days
        ...
        Parkorbit1   = Planet(a=1.2,    e=0.017, T=1.45,    m=3.003486962,  sim=gui.Parkorbit1.get(),   c ='DarkBlue')
        Mars    = Planet(a=1.524, e=0.094, T=1.88,  m=0.3232371722, sim=gui.mars.get(),    c ='Crimson')
        Jupiter = Planet(a=5.200, e=0.049, T=11.860,m=954.7919,     sim=gui.jupiter.get(), c ='orange')
        ...
        Pluto = Planet(a=39.5, e=0.011, T=164.8, m=51.51384,     sim=gui.pluto.get(), c ='Brown')
...
class Spacecraft(Planet):
    ...
    def calculateInitialVelocity(self, delta):
        ...
        return (velocity*np.cos(self.eccentricAnomaly) - self.origin.position[0]/self.origin.distanceToSun()**3 * delta/2,
                velocity*np.sin(self.eccentricAnomaly) - self.origin.position[1]/self.origin.distanceToSun()**3 * delta/2)
    ...
    def performOrbitInsertion(self, Sim):
        if not Sim.Planets['HTO'].orbitInserted:
            ...
            requiredAcceleration = np.sqrt(1/semiMajorDestination) * (1 - np.sqrt(2 * semiMajorOrigin / (semiMajorOrigin + semiMajorDestination))) # x-component
...
class App:
    ...
    def __init__(self, master):
        ...
        self.planet_frame.grid(row=1, columnspan=6, sticky='EW')
        
        # Planets
        self.mercury = tk.BooleanVar()
        self.venus = tk.BooleanVar()
        self.earth = tk.BooleanVar()
        self.Parkorbit1 = tk.BooleanVar()
        ...
        self.plot_Parkorbit1 = ttk.Checkbutton(self.planet_frame, text='Park Orbit-1', variable=self.Parkorbit1)
        self.plot_Parkorbit1.grid(row=4, column=0, sticky='EW', padx=5, pady=5)
        self.plot_mars = ttk.Checkbutton(self.planet_frame, text='Mars', variable=self.mars)
        self.plot_mars.grid(row=1, column=1, sticky='EW', padx=5, pady=5)
        self.plot_jupiter = ttk.Checkbutton(self.planet_frame, text='Jupiter', variable=self.jupiter)
        self.plot_jupiter.grid(row=2, column=1, sticky='EW', padx=5, pady=5)
        self.plot_saturn = ttk.Checkbutton(self.planet_frame, text='Saturn', variable=self.saturn)
        self.plot_saturn.grid(row=3, column=1, sticky='EW', padx=5, pady=5)
        self.plot_uranus = ttk.Checkbutton(self.planet_frame, text='Uranus', variable=self.uranus)
        self.plot_uranus.grid(row=4, column=1, sticky='EW', padx=5, pady=5)
        self.plot_neptune = ttk.Checkbutton(self.planet_frame, text='Neptune', variable=self.neptune)
        self.plot_neptune.grid(row=1, column=2, sticky='EW', padx=5, pady=5)
        self.plot_pluto = ttk.Checkbutton(self.planet_frame, text='Pluto', variable=self.pluto)
        self.plot_pluto.grid(row=2, column=2, sticky='EW', padx=5, pady=5)
        ...
        self.plot_button = ttk.Button(master, text='Suggest Simulation Parameters', command=self.suggestSimParameters)
        self.plot_button.grid(column=1, row=12, sticky='EW')
        
        self.plot_button = ttk.Button(master, text='Plot', command=self.plot)
        self.plot_button.grid(column=1, row=13, sticky='EW')
    ...
    def plot(self):
        ...
        sim_window.title('Hohmann Transfer Orbit')
    ...
    def simulation_animation(self):
        ...
        # Create new window to plot the simulation
        sim_window = tk.Toplevel()
        sim_window.title('Hohmann Transfer Team_25')
        sim_window.focus_force()    # Auto-focus on window
...
if __name__ == '__main__':
    ...
    # Some definitions
    planets = ['Pluto','Neptune', 'Uranus', 'Saturn', 'Jupiter', 'Mars','Parkorbit1', 'Earth', 'Venus', 'Mercury']

    # ------
    # Start GUI
    root = tk.Tk()
    root.wm_title('Hohmann Transfer Team 25')
    gui = App(root)
    root.mainloop()